...
describe('OllamaBackend', () => {
	...
	describe('predict - chat mode', () => {
		...
		test('should handle chat API errors', async () => {
			...
			await assert.rejects(async () => backend.predict('error-model', { prompt: 'test' }), /Ollama API error: 500/);
		});
	...
	describe('predict - embeddings mode', () => {
		...
		test('should handle embeddings API errors', async () => {
			...
			await assert.rejects(async () => backend.predict('missing-model', { prompt: 'test' }), /Ollama API error: 404/);
		});
