...
describe('BenchmarkEngine', () => {
	...
	describe('constructor', () => {
		...
		it('should throw error if inference engine is not provided', () => {
			assert.throws(() => new BenchmarkEngine(), /inference engine is required/i);
		});
	});
	...
	describe('findEquivalentModels', () => {
		...
		it('should return empty array if no models match', async () => {
			const found = await benchmarkEngine.findEquivalentModels('nonexistent-task', 'nonexistent-group');
	...
	describe('validateModelsForComparison', () => {
		...
		it('should throw error if less than 2 models', async () => {
			...
			assert.throws(() => benchmarkEngine.validateModelsForComparison(models), /at least 2 models/i);
		});
		...
		it('should throw error if metadata is missing', async () => {
			...
			assert.throws(() => benchmarkEngine.validateModelsForComparison([model, model]), /missing.*outputDimensions/i);
		});
	});
	...
	describe('compareBenchmark', () => {
		it('should run benchmark and return results', async () => {
			...
			const result = await benchmarkEngine.compareBenchmark(models, testData, options);
		...
		it('should identify winner with lowest avgLatency', async () => {
			...
			const result = await benchmarkEngine.compareBenchmark([model1, model2], testData, {
				iterations: 10,
				taskType: 'text-embedding',
				equivalenceGroup: 'test',
			});
	...
	describe('getHistoricalResults', () => {
		it('should retrieve historical results by taskType', async () => {
			...
			assert.ok(historical.length >= 1, 'Should have at least one historical result');
