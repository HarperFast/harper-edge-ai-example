...
describe('Benchmark E2E Workflow', () => {
	...
	test('complete workflow: upload → benchmark → use winner', async () => {
		...
		assert.ok(history.length >= 1, 'Should have at least 1 historical result');
		...
		assert.ok(personalizationEngine.isReady(), 'Personalization engine should be ready');
		...
		for (const product of enhanced) {
			assert.ok(product.personalizedScore !== undefined, 'Product should have personalizedScore');
			assert.equal(product.personalized, true, 'Product should be marked as personalized');
		}
