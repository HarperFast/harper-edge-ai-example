...
describe('Benchmark API Integration', () => {
  ...
  describe('POST /benchmark/compare', () => {
    test('should compare equivalent models successfully', async () => {
      ...
      assert.ok(
        foundModels.length >= 2,
        'Should find at least 2 equivalent models'
      );
    ...
    test('should return error if not enough models found', async () => {
      ...
      assert.equal(
        foundModels.length,
        1,
        'Should only find 1 model (not enough)'
      );
    ...
    test('should validate output dimensions match', async () => {
      ...
      // Fetch both models
      const allModels = await benchmarkEngine.findEquivalentModels(
        'text-embedding',
        'mismatched'
      );
    ...
    test('should store result in BenchmarkResult table', async () => {
      ...
      const { createMockInferenceEngine } = await import(
        '../helpers/benchmark-helpers.js'
      );
