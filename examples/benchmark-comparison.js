...
async function useWinningModel(winner, products, userContext) {
	...
	const response = await fetch(`${BASE_URL}/personalize?modelId=${modelId}&version=${version}`, {
		method: 'POST',
		headers: { 'Content-Type': 'application/json' },
	...
	result.products.forEach((product, idx) => {
		console.log(`${idx + 1}. ${product.name} (score: ${product.personalizedScore.toFixed(3)})`);
	});
...
async function main() {
	try {
		...
		const products = [
			{
				...
				description:
					'Lightweight trail running shoes with breathable mesh and cushioned sole for long-distance comfort',
				category: 'Footwear',
				price: 129.99,
		...
		const personalizationResult = await useWinningModel(winner, products, userContext);

		// Example 5: Query history
		console.log('\n\nStep 5: Query Benchmark History');
		const history = await getBenchmarkHistory('text-embedding', 'product-recommender');
		...
		if (history.results.length > 0) {
			...
			history.results.slice(0, 3).forEach((r, idx) => {
				...
				console.log(`  ${idx + 1}. ${r.id} - ${date} (${r.iterations} iterations)`);
			});
		}
